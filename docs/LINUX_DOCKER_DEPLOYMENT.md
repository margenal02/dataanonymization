...
PP-StructureV3 精简流水线和 UIE-base 都运行在同一个后端容器内，不会另装一套 Docker。扫描 PDF 时先启动独立 OCR 子进程，完成后释放 OCR 模型，再按 Web 选项临时加载或常驻 UIE-base。“临时调用”适合 16～31 GB 内存，“模型常驻”建议至少 32 GB；两种模式使用同一个已封装进镜像的 UIE-base。
...
首次构建后端镜像会安装 PaddlePaddle、PaddleOCR/PaddleX、PaddleNLP 和 Poppler，并预取 PP-StructureV3 精简模型与 UIE-base，耗时和镜像体积会明显增加。精简 OCR 只加载版面、移动端文字检测和移动端文字识别，明确关闭表格、公式、印章、图表、文档矫正及区域检测。扫描 PDF 的页面渲染与 OCR 全部在后端容器内逐页执行，默认限制为 300 个 OCR 页面、单页 180 秒；Web 页面会显示实时页码和进度。构建成功后任务推理与 OCR 不需要访问外部模型服务。若生产网络禁止访问公共源，应先在联网构建机完成镜像构建与漏洞扫描，再通过单位批准的 Harbor/制品库离线导入目标主机。
