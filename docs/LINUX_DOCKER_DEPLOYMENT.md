...
首次构建后端镜像会安装 PaddlePaddle/PaddleNLP、Poppler、Tesseract 简体中文/英文语言包并预取 UIE-micro，耗时和镜像体积会明显增加。扫描 PDF 的页面渲染与 OCR 全部在后端容器内逐页执行，默认限制为 300 个 OCR 页面、单页 180 秒；Web 页面会显示实时页码和进度。构建成功后任务推理与 OCR 不需要访问外部模型服务。若生产网络禁止访问公共源，应先在联网构建机完成镜像构建与漏洞扫描，再通过单位批准的 Harbor/制品库离线导入目标主机。
