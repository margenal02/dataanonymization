...
脚本每 0.5 秒读取断点文件的实际增长量，因此可以显示真实下载大小、百分比、瞬时速度和平均速度；即使暂时没有新字节，也会显示 `实时 0.00 MB/s`。Web 直连失败时才自动切换 Microsoft Store 系统通道；系统缺少 `curl.exe` 时自动回退到 .NET 下载器。
...
Web 直连的进度由 HTTP 响应中的总大小和实际写入磁盘的字节数计算，每 0.5 秒刷新一次；速度由对应时间窗口内的实际字节增量计算，不根据时间虚构。固定使用 `Store` 时，下载由 Windows 系统组件接管；如果系统通道不返回字节数据，脚本只能显示阶段状态，无法计算可靠的 MB/s。
