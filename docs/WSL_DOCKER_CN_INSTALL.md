...
首次执行“构建 Django 与 UIE-micro 后端镜像”时，会通过选中的 PyPI 镜像安装 PaddlePaddle/PaddleNLP，并下载约 90 MB 的 UIE-micro 模型文件和生成本地推理缓存。模型缓存被封装进后端 Docker 镜像，业务任务执行时不需要把文件上传给外部模型，也不会为“临时调用”重新下载模型。该构建步骤通常比其他步骤耗时更长；重复安装且代码、依赖未变化时 Docker 会复用缓存。
