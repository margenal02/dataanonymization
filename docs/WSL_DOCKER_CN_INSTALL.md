...
脚本需要管理员权限；如果从普通 PowerShell 启动，会自动打开管理员窗口。该窗口在检测或安装结束后保持打开，便于查看结果和错误日志，可确认完成后手动关闭。部署完成时脚本会优先创建并立即启动 Windows 计划任务 `DataAnonymizationWslKeepAlive`：该任务在当前用户登录后自动启动 WSL、Docker 和本项目，并用一个不含密码或密钥的常驻进程避免 WSL 因空闲退出。任务没有执行时限，在使用电池时也不会被系统自动停止。若组织安全策略以 `0x80070005` 等错误拒绝创建计划任务，脚本会自动启动隐藏的当前登录会话后台进程保活，不会在应用已经部署完成后继续启动一个不存在的任务。该降级方式在本次登录期间有效；Windows 重启或注销后需重新运行脚本，或由管理员放行计划任务创建权限。

首次启用 WSL2 时需要重启 Windows。脚本会先尝试写入系统级一次性 `RunOnce` 续跑项；若系统策略拒绝，则自动改用当前用户级续跑项。手动重启并登录原管理员账号后，安装会自动继续，且不会重复询问是否安装。全部完成后续跑项会自动清除。如果两种续跑项都被策略禁止，脚本不会把安装判定为失败，而会提示重启后以管理员身份重新运行同一个脚本；已完成步骤会自动跳过。

在部分 Windows 10 主机上，启用系统组件后安装新版 WSL MSI 还会返回 `3010`，表示安装成功但必须再次重启。脚本会识别该状态并停止在正确位置，不会提前安装 Ubuntu。重启续跑时，已下载并校验的 WSL MSI 和 Ubuntu 安装文件都会直接复用。
...
脚本会把检测选中的实际地址写入 WSL 配置和 `.env`，不是只在检测阶段临时放行；已有 `.env` 的密码与密钥保持不变，只更新镜像、UIE 组件参数和旧版上传上限，确保失败后重试也能应用新版本。国内公共镜像属于外部依赖，服务地址可能调整。生产环境建议将所需镜像同步到组织自有的 Harbor/制品库，并将 `.env` 中的四个镜像地址改为内部地址。

脚本同时写入 `REQUIRE_HUMAN_REVIEW=1`。上传任务识别结束后进入“待人工确认”，完整提取文本按页/片段预览，候选项的所有相同出现位置按类型着色；系统还会列出可能同指一个单位的全称/简称供人工决定是否共用匿名代码。只有人工确认过的项目才会写入最终匿名映射，未确认任务不能下载脱敏文件。生产环境不建议关闭此项。
...
扫描 PDF 会显著占用 CPU。部署默认使用 `PDF_OCR_MODE=fast`，只运行移动版文字检测和识别；Web 可对复杂版面切换 `layout`。默认最长处理 300 个 OCR 页面，页面渲染为 150 DPI、最长边限制 2480 像素，单页渲染或识别超时为 180 秒，首次加载 OCR 最长等待 600 秒。处理过程中 Web 显示渲染、模型加载、当前 OCR 页与百分比，并可中断当前脱敏任务；重新人工校正会复用首次 OCR 结果。安装脚本会把对应 `PDF_OCR_*` 和 `PPSTRUCTURE_*` 参数写入新旧 `.env`，不会修改既有密码和密钥。
...
新版默认单文件上传上限为 200 MB。更新已有部署时，安装脚本只会把旧版未修改过的 `MAX_UPLOAD_SIZE_MB=50` 自动迁移到 `200`；管理员自行设置的其他数值保持不变。部署阶段会重建 Nginx 入口容器，使新的上传限制和 JSON 格式超限提示立即生效，不删除数据库、任务记录或已保存文件。
...
安装脚本不会在第一次请求成功后立即结束，而会从 Windows 对两个地址执行至少 5 次连续健康检查。连续约 15 秒均可访问、且实际选用的保活方式仍处于运行状态时，才会显示“安装和部署全部完成”。通常使用 `DataAnonymizationWslKeepAlive` 计划任务；若被 Windows 策略拒绝，则验证当前登录会话的隐藏后台保活进程。微软明确说明，systemd 服务本身不会使 WSL 实例保持运行，因此不要删除或停止已经创建的计划任务。降级为当前会话保活时，完成信息会用黄色提示 Windows 重启或注销后的恢复方法。
...
如果上述查询提示任务不存在，但安装完成信息显示“当前登录会话后台进程”，说明本机安全策略禁止了计划任务，应用在本次登录期间仍会保持运行。Windows 重启或注销后，以管理员身份重新运行同一个安装脚本即可恢复；Docker 构建缓存和现有数据卷都会复用。
...
当前脚本在 WSL 内的 Docker 准备阶段使用 UTF-8 locale，并通过结构化进度标记显示“软件源、Python 镜像、Docker 安装、容器镜像、systemd”5 个小步骤。Windows PowerShell 5.1 可能把重定向的 WSL 中文再次按系统代码页解码，因此动态进度中的“最近输出”只追加 ASCII 诊断；完整原始输出仍保存在支持日志中。这可避免进度条尾部出现乱码，同时不丢失故障证据。
...
WSL 内 Docker 准备阶段先显示 `步骤 1/5` 至 `步骤 5/5`；随后容器部署阶段显示 `步骤 1/11` 至 `步骤 11/11`，依次覆盖 Docker 启动、安全配置、基础镜像拉取、后端构建、前端构建、MySQL 启动与检查、Django 启动与检查、Nginx 启动和网站检查。同一小步骤耗时较长时，单行进度会每秒更新本步骤耗时、部署总耗时和最近一条安全诊断；数据库及后端健康检查还会显示当前检查次数、容器状态和健康状态。当前步骤同时写入 `.runtime` 下的临时状态文件，Docker 构建日志很大时也不会丢失步骤名称，流程结束后自动删除该文件。
