...
本地识别器覆盖烟草单位全称、常见部门、角色上下文人名、人员名单、表格独立姓名、电话、证件、邮箱和规范地址，并兼容 PDF 提取后文字间出现空格、Word 文本框以及 OFD `TextCode` 拆分等常见情况。规则无法判断的简称或内部代号可通过页面“指定敏感词”补充；命中数量为 0 时不要直接把输出文件当作有效脱敏结果。
