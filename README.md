...
WSL 本身只使用 Microsoft Store/GitHub 官方通道；安装窗口显示 `[进度条] 百分比 当前阶段`。命令返回真实下载百分比时会实时更新；没有返回时停留在当前阶段起点，不使用时间伪造进度。Microsoft Store 长时间停在 0% 时可改用 `-WslDownloadChannel Web`。Ubuntu、Docker、PyPI、npm 和应用容器继续使用大陆镜像。
