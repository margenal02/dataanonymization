...
WSL 系统包只使用微软官方来源；`Auto`/`Web` 会先对 GitHub 官方直连及大陆反向代理做小文件实测，验证文件头后选择最快通道，再通过 Windows 原生 `curl.exe` 下载微软签名的 WSL MSI 和微软清单中的 Ubuntu 包。窗口每 0.5 秒在同一个动态进度区域内更新真实百分比、已下载/总大小、瞬时及平均 MB/s，不会连续新增下载记录，并支持断点续传和缓存复用。首次系统资源检测拆分为 10 个可见小步骤，逐项记录 WMI、DISM、端口和镜像源选择。软件包源会在清华、阿里云和 USTC 之间自动回退，npm 与容器镜像也会在国内源不可用时尝试官方源；单个候选超时不会再终止整个安装。容器部署另拆分为 11 个可见小步骤，持续显示已等待时间、最近的 Docker 输出及健康检查次数。代理传输内容仍必须通过官方 SHA-256 和微软数字签名校验。现有 WSL、发行版和 Docker 检测有 12～20 秒超时保护；旧版 `wsl.exe` 的 UTF-16/系统代码页输出也会自动解码，避免中文乱码。
