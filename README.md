...
本地识别器覆盖烟草单位全称、常见部门、负责人/组长/成员/评委等角色上下文人名、人员名单、表格独立姓名、电话、证件、邮箱和规范地址，并兼容 PDF 提取后文字间出现空格、Word 文本框以及 OFD `TextCode` 拆分等常见情况。正文和文件名共用可逆映射：脱敏文件名使用 `ANON_类别_任务码_序号` 安全标记，正式文件名在反匿名时恢复。规则无法判断的简称或内部代号可通过页面“指定敏感词”补充；命中数量为 0 时不要直接把输出文件当作有效脱敏结果。
